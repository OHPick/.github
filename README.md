...
| **해결 방안** | 1. Axios만을 이용한 데이터 통신
2. Axios와 React Query를 함께 사용하여 서버 상태 관리 |
| **의견 조율 및 의정** | ⇒ `성능을 위해 캐싱 기능에 대한 고려가 필요` 
...
react-query를 사용하게 되면 캐싱 데이터를 사용하여 중복되는 데이터 요청을 줄이고, 
필요할 때에 캐싱된 데이터와 새로 업데이트 된 데이터를 새로 가져올 수도 있습니다. 
이렇게 캐싱 처리된 데이터를 사용하게 되면 사용자가 빠르게 데이터를 확인할 수 있기 때문에 
사용자 경험성이 향상될 것으로 판단하여 react-query를 도입하였습니다. |
